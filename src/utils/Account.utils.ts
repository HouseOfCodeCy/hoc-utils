...
export const printAddressAsString = (address: IAddress | undefined) => {
	...
	if (address) {
		...
		if (address.attributes.address2)
			addressString += `${address.attributes.address2}, `;
		...
		if (address.attributes.city.data.attributes.country.data)
			addressString += `${address.attributes.city.data.attributes.country.data.attributes.name}`;
	}
